...
class Pdf3ColGenerator:
    ...
    def generate(self, client_name, start_date, steps_dict):
        file_path = os.path.join(self.output_dir, f"{client_name.replace(' ', '_')}_3col.pdf")
        ...
        # === Columns ===
        c.setFont("Helvetica", 10)
        col_width = (width - 2 * left_margin + 10) / 3
        ...
        max_steps = max(len(col1_data), len(col2_data), len(col3_data))
        row_heights = []
        ...
        for i in range(max_steps):
            ...
            # Determine if this row should be shaded in each column
        ...
        c.setLineWidth(0.2)
        c.setStrokeColorRGB(0, 0, 0)
        for x in [col1_x - 10, col1_x + wrap_width + 10, col2_x + wrap_width + 10]:
            c.line(x, table_top + 25, x, current_y + 10)
        line_y = table_top
        for height in row_heights[:-1]:
            line_y -= height
            c.line(col1_x - 65, line_y + 10, col3_x + wrap_width + 10, line_y + 10)
        c.line(col1_x - 65, table_top + 10, col3_x + wrap_width + 10, table_top + 10)
